/**
 * The Common Paper Mutual NDA Standard Terms, Version 1.0, as structured data.
 *
 * Transcribed verbatim from `templates/Mutual-NDA.md` in the repository root.
 * Inline references such as "Purpose" or "Governing Law" are *defined terms*
 * that point at the Cover Page by design — they are intentionally NOT
 * placeholders and are left as-is.
 *
 * This is the single source of truth consumed by both the HTML preview
 * (`NdaPreview`) and the PDF document (`NdaPdfDocument`).
 */

export interface StandardTermClause {
  /** Clause number, e.g. 1. */
  number: number;
  /** Bold lead-in title, e.g. "Introduction". */
  title: string;
  /** Body text of the clause (single paragraph). */
  body: string;
}

export const STANDARD_TERMS_HEADING = "Standard Terms";

export const STANDARD_TERMS: StandardTermClause[] = [
  {
    number: 1,
    title: "Introduction",
...
export const STANDARD_TERMS_ATTRIBUTION =
