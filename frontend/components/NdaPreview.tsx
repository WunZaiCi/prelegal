...
function Value({ children, placeholder }: { children: string; placeholder: string }) {
  ...
  return (
    <span className="italic text-oxblood/55">[{placeholder}]</span>
  );
}
...
function PartyColumn({ party }: { party: NdaParty }) {
  const row = (label: string, value: string, ph: string) => (
    <div className="border-t border-line/70 py-2">
      <div className="font-ui text-[9px] uppercase tracking-[0.14em] text-ink-soft">
        {label}
      </div>
...
export default function NdaPreview({ data }: { data: NdaFormData }) {
  return (
    <article className="mx-auto max-w-[640px] bg-[#FCFAF4] px-10 py-12 font-body text-[14px] leading-relaxed text-ink shadow-document sm:px-14 sm:py-16">
      {/* ── Cover Page ── */}
      <header className="text-center">
        <p className="font-ui text-[10px] uppercase tracking-[0.32em] text-oxblood">
          Cover Page
        </p>
        <h1 className="mt-3 font-display text-[28px] font-600 leading-tight text-ink">
      ...
      {/* Signature table */}
      <div className="mt-6 flex flex-col gap-6 rounded-md border border-line bg-paper/40 p-5 sm:flex-row">
      ...
      <div className="mt-14 border-t-2 border-double border-ink/30 pt-12">
        <h2 className="text-center font-display text-[22px] font-600 text-ink">
          {STANDARD_TERMS_HEADING}
        </h2>
...
function CoverField({
  ...
  return (
    <div>
      <dt className="font-ui text-[11px] font-700 uppercase tracking-[0.16em] text-ink">
        {label}
      </dt>
      {note ? (
        <dd className="mt-0.5 text-[12px] italic text-ink-soft">{note}</dd>
      ) : null}
...
function Attribution() {
  return (
    <p className="mt-10 text-center font-ui text-[10px] leading-relaxed text-ink-soft/80">
      {STANDARD_TERMS_ATTRIBUTION}
    </p>
  );
}
